...
describe("event source mapping", () => {
  ...
  test.skip("verify that alias.addEventSourceMapping produces stable ids", () => {
    // GIVEN
    var alias = new compute.Alias(spec, "LiveAlias", {
      aliasName: "Live",
      function: fn,
      version: fn.version,
    });
  ...
  test("throws if maxBatchingWindow > 300 seconds", () => {
    expect(
      () =>
        new compute.EventSourceMapping(spec, "test", {
          target: fn,
          eventSourceArn: "",
  ...
  //   const spec2 = new AwsSpec(Testing.app(), "Spec2", {
  ...
  //   const topicNameParam = new TerraformVariable(spec, "TopicNameParam", {
  //     type: "String",
  //   });
