...
// copied from https://github.com/aws/aws-cdk/blob/v2.161.1/packages/aws-cdk-lib/aws-iam/lib/private/merge-statements.ts
//
// See https://github.com/aws/aws-cdk/blob/v2.161.1/packages/aws-cdk-lib/aws-iam/docs/policy-merging.als
// for a formal model of the logic implemented here.
...
export function mergeStatements(
  ...
  const mergeFn =
    (options?.mergeIfCombinable ?? true) ? mergeIfCombinable : mergeIfEqual;
...
function mergeIfCombinable(
  ...
  const combined = a.statement.copy({
    ...
    principals: setMergePrincipals(
      a.statement.principals,
...
function makeComparable(s: PolicyStatement): ComparableStatement {
  return {
    ...
    ...(s.conditions
      ? {
          conditionString: JSON.stringify(toConditionJson(...s.conditions)),
        }
      : {}),
  };
}
...
function unmergeablePrincipals(a: ComparableStatement, b: ComparableStatement) {
  const aHasStarPrincipal = a.statement.principals.some(
    (v) =>
      typeof v.policyFragment.principalJson === "string" &&
      v.policyFragment.principalJson == "*",
  );
  ...
  return aHasStarPrincipal !== bHasLiteral;
}
...
function setEqualPrincipals(xs: IPrincipal[], ys: IPrincipal[]): boolean {
  ...
  const nonComp = setEqual(
    xPrincipals.nonComparable,
    yPrincipals.nonComparable,
  );
  const comp = setEqual(
    Object.keys(xPrincipals.comparable),
    Object.keys(yPrincipals.comparable),
  );
